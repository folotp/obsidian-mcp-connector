...
> **Aggiornato 2026-05-16 — STANDALONE pivot completato + Obsidian community store ACCEPTED (`0.4.8` reproducible-build fix): sessione di chiusura. Promote `0.4.x`→`main` + de-fork da `jacksteamdev` finalizzati (PR [#112](https://github.com/istefox/obsidian-mcp-connector/pull/112)/[#113](https://github.com/istefox/obsidian-mcp-connector/pull/113)/[#115](https://github.com/istefox/obsidian-mcp-connector/pull/115), tag `0.4.7`, rollback `archive/main-0.3.12`, remote upstream rimossi, LICENSE MIT + CHANGELOG preservati). **GitHub fork-network detach → CLOSED won't-do**: i fork figli (`AnhNguyenz`/`ezrahill`) bloccano il self-service e l'unico path GitHub (delete+recreate) distruggerebbe TUTTE le Release → romperebbe BRAT, sproporzionato; Support AI deflection conferma nessun detach manuale non-distruttivo; accettato il badge cosmetico "forked from" (NON ha bloccato l'accettazione store → decisione vindicata). `feat/http-embedded` **RITIRATO** (0-ahead/29-behind main; rimosso dal ruleset `General` prima della delete, protezione main intatta). #100 Windows **field-corroborated** da `@nathancrum` su PR [#104](https://github.com/istefox/obsidian-mcp-connector/pull/104) (sostituzione dead-value funziona su Windows). **Saga store**: review portale `0.4.7` Completed = tutto Pass / disclosure-intrinseca TRANNE **BUILD VERIFICATION failed** ("main.js da sorgente ≠ release artifact"). Root cause: `onnxruntime-web` risolve la dist da `__dirname`/`__filename` → con `target:"node"` Bun incollava il path della macchina di build (`/home/runner/...`) nel bundle (stessa classe di #100). Fix in **`0.4.8`** (PR [#116](https://github.com/istefox/obsidian-mcp-connector/pull/116) squash→`ed19351c`, tag `0.4.8`): neutralizzati `__dirname`/`__filename` nel `define` di `bun.config.ts` + pin bun `latest`→`1.3.12` (`release.yml`+`mise.toml`). Verificato: `0.4.8` pubblicato **byte-identico** a un rebuild da sorgente indipendente (riproducibile cross-machine) + 0 path macchina. Review `0.4.8`: **Satisfactory 89%, BUILD VERIFICATION Pass**, "Add to Obsidian" attivo; Warning `fs`/`child_process` rimaste **non bloccanti** (disclosure intrinseche — scelta di NON sventrare le feature vindicata; la Long description del listing già giustifica). Dependabot: PR [#114](https://github.com/istefox/obsidian-mcp-connector/pull/114) (vite 5→8) **chiusa** (pin via root `overrides`), `.github/dependabot.yml` **ripristinato v5** (PR [#117](https://github.com/istefox/obsidian-mcp-connector/pull/117) — era stato perso nel de-fork, referenziava il rimosso `mcp-server`; aggiunto total-ignore `vite`). **OUTCOME**: plugin **LISTATO** in `obsidianmd/obsidian-releases/community-plugins.json` 2026-05-16T15:35Z (id `mcp-tools-istefox`) → trovabile/installabile dal browser community in-app, con disclaimer standard Obsidian "This plugin has not been manually reviewed by Obsidian staff" (path automatico + capacità high-risk; rimosso solo da un eventuale audit umano Obsidian — fuori dal nostro controllo). Stato terminale della saga store.**
>
> Storico (precedente): **2026-05-15 mattina (folotp bug burst overnight: 3 NEW issues filed in 24h + triage batch posted, fix pending Stefano stasera): folotp ha aperto 2 nuove issue ieri sera (2026-05-14): [#98](https://github.com/istefox/obsidian-mcp-connector/issues/98) `pre-warm: mcp-remote --help throws ERR_INVALID_URL on Node 24` (LOW, root cause upstream `mcp-remote` parseCommandLineArgs su Node 24, plugin già detect+success ma stderr stack trace ancora visible) + [#99](https://github.com/istefox/obsidian-mcp-connector/issues/99) `search_vault_smart ignores semanticSearch.provider:"smart-connections" — always invokes native Transformers.js pipeline` (HIGH, handler `searchVaultSmart.ts` bypassa il `providerFactory` esistente + va direct a native `ensurePipeline`/`from_pretrained` regardless of setting; misleading "not ready" error). Plus [#96](https://github.com/istefox/obsidian-mcp-connector/issues/96) ancora pending (delete_vault_file trash bypass, già scoped 2026-05-13 mid-day). **Triage batch posted 2026-05-15** ([#96 comment 4457246546](https://github.com/istefox/obsidian-mcp-connector/issues/96#issuecomment-4457246546) + [#98 comment 4457246967](https://github.com/istefox/obsidian-mcp-connector/issues/98#issuecomment-4457246967) + [#99 comment 4457247133](https://github.com/istefox/obsidian-mcp-connector/issues/99#issuecomment-4457247133)) — tutti committed publicly come MIA ownership del fix-batch (`fix/96-honours-trash-setting`, etc.); marcoaperez non taggato in nessuno (conforme [[feedback-no-contributor-delegation-obsidian-mcp]]). **Code investigation done** preliminare per #98 (`preWarm.ts:138/164/192`) e #99 (`searchVaultSmart.ts` + `semantic-search/services/providerFactory.ts` + `plugin.semanticSearchState.provider` esistente in `main.ts:86`); fix surface pinned nei triage drafts. Stefano farà fix batch stasera a casa: drafts saved a `/tmp/issue96-triage.md` + `/tmp/issue98-triage.md` + `/tmp/issue99-triage.md` per reference. **Folotp engagement burst observation**: 3 bug filed in 24h, tutti con triage-grade detail (root cause traced, suggested fix, environment, repro steps). Engagement quality consistente high. Marcoaperez silenzio 48h post-#97 merge (normal stochastic). Store PR #11919 invariato 2026-05-07 (week 6/8). Upstream `jacksteamdev` silent push 2026-05-13 16:11-16:13Z (0.2.32 + lockfile + 0.2.33 + readme update) noto, no movement since.**
>
...
>
> Aggiornato `0.4.0` → … → `0.4.6` → **`0.4.7`** (STANDALONE: promote `0.4.x`→`main` + de-fork completo da `jacksteamdev`) → **`0.4.8`** (reproducible-build fix per la review Obsidian store) shipped consecutivamente. Il progetto è ora **standalone** (de-forkato a livello di contenuto/remote/doc; il distacco fork-network GitHub è chiuso won't-do) ed è **accettato e listato** nel registro community Obsidian (portale + in-app, con disclaimer auto-review). Documento di passaggio di consegne. Self-contained.
>
...
## Decisioni di sessione 2026-05-16 — STANDALONE pivot + Obsidian store ACCEPTED (`0.4.8` reproducible-build fix) 🏁

**Trigger**: continuazione sessione post-2026-05-15. Inizio: consegna del report finale del promote `0.4.x`→`main` + de-fork (fasi pianificate concluse). Poi push completo verso l'accettazione nel community store Obsidian, fino al listing in-app.

### 1. Promote `0.4.x`→`main` + de-fork standalone (finalizzato)
`main` = linea standalone in-process `0.4.7` (PR #113 merge dual-parent, tree byte-identico all'ex `feat/http-embedded`; PR #112 de-fork contenuto/doc/remote; PR #115 CLAUDE.md). `0.3.x` abbandonata, archiviata in `archive/main-0.3.12` (HEAD `76fa012`). LICENSE MIT + storia CHANGELOG preservate (vincolo legale). Remote `upstream`/`marcoaperez` rimossi. Tag/release `0.*` immutabili (ruleset). Dettaglio in `docs/plans/2026-05-16-promote-0.4-to-main-and-destandalone.md` + memoria auto.

### 2. GitHub fork-network detach → CLOSED won't-do
Verificato via API: repo ancora `fork:true` con **2 fork figli** (`AnhNguyenz/obsidian-mcp-connector`, `ezrahill/obsidian-mcp-connector`, inattivi) → "Leave fork network" self-service disabilitato. L'unico path GitHub per il caso "child forks" è *delete repo + recreate + `git push --mirror`*: `--mirror` ripristina solo i ref git, **NON** le GitHub Releases (oggetti+asset = metadati GitHub) → distruggerebbe TUTTE le release incl. `0.4.7`/`0.4.8`, rompendo BRAT per tutti gli utenti, e violerebbe il vincolo hard "mai distruggere release/tag `0.*`". Lo schermo AI di GitHub Support conferma: nessun detach manuale non-distruttivo. **Decisione (Stefano)**: accettare il badge cosmetico "forked from". Esito empirico successivo: il fork-status **NON** ha bloccato l'accettazione store → decisione vindicata.

### 3. `feat/http-embedded` RITIRATO
`compare main...feat/http-embedded` = `ahead_by:0, behind_by:29` → nulla da perdere. Il ruleset `General` proteggeva `refs/heads/feat/http-embedded` con regola `deletion`; modificato chirurgicamente a `["~DEFAULT_BRANCH"]` (protezione `main` intatta: `deletion`+`non_fast_forward`) → branch cancellato. Branch superstiti: `main`, `archive/main-0.3.12`.

### 4. #100 Windows — field-corroborated
`@nathancrum` (reporter Windows) ha commentato su PR [#104](https://github.com/istefox/obsidian-mcp-connector/pull/104): ha cancellato a mano la riga `fileURLToPath(import.meta.url)` lasciando `'./'` → "works fine on Windows". Conferma sul campo la premessa del fix #100 (valore *morto* nel nostro build; solo il throw contava). Risposto su #104 (loop chiuso col reporter): la `0.4.7` rilasciata già spedisce il fix (placeholder drive-letter). Rischio residuo #100 → molto basso, **non bloccante**; Stefano non può fare lo smoke Windows in proprio (accettato).

### 5. Saga store Obsidian — review `0.4.7`
Submission migrata al portale `community.obsidian.md` (vecchio PR #11919 chiuso, processo deprecato). "Review branch" produce un Preview diagnostico; il path ufficiale è "Check for new releases" → "Request review". Review `0.4.7` (commit `f518ef3`) **Completed**: RELEASES attestation Pass, DEPENDENCIES Pass, Vault R/W Pass; BEHAVIOR = disclosure-intrinseche (`fs`=1 `existsSync`; `child_process`=lazy node-detect+prewarm+brew-install già one-click; vault-enum/clipboard write-only/dynamic-code-exec da dipendenza bundellata — NON difetti, NON da rimuovere); unico fallimento reale **BUILD VERIFICATION** ("main.js built from source does not match the release artifact").

### 6. Root cause + fix → `0.4.8` (systematic-debugging)
Evidenza diretta: il `main.js` rilasciato `0.4.7` conteneva incollato `/home/runner/work/.../node_modules/onnxruntime-web/dist/ort-web.node.js`. Causa: `onnxruntime-web/dist/ort-web.node.js` (CJS) risolve la dist da `__dirname`/`__filename`; con `target:"node"` Bun li sostituisce col path assoluto della macchina di build → bundle non riproducibile cross-machine (stessa classe di #100, identificatore diverso; il `define` neutralizzava solo `import.meta.url`/`filename`). **Fix (PR [#116](https://github.com/istefox/obsidian-mcp-connector/pull/116), squash→`ed19351c`, tag `0.4.8`)**: aggiunti `"__dirname"`/`"__filename"` al `define` di `packages/obsidian-plugin/bun.config.ts` (valore morto: onnxruntime gira WASM/CDN-pinned, `allowLocalModels=false`, `onnxruntime-node` shimmato) + pin toolchain `latest`→`1.3.12` in `.github/workflows/release.yml` e `mise.toml`. Verifica: due build prod locali byte-identiche (deterministico); `0.4.7` rilasciata = 1 occorrenza `home/runner` → nuovo build = **0**; `0.4.8` pubblicato dalla CI = **byte-identico** (`cmp -l` = 0) a un rebuild da sorgente indipendente su macchina diversa = ciò che il reviewer verifica.

### 7. Review `0.4.8` + scorecard
`0.4.8`/`ed19351c` **Completed**: **BUILD VERIFICATION Pass** ("main.js reproduced byte-for-byte from source"), score listing **89%** (Health Excellent, Review Satisfactory), "Add to Obsidian" attivo. BEHAVIOR Warning `fs`/`child_process` + 3 Recommendation **rimaste e non bloccanti**. Scorecard analizzato: i "5 issues" = 2 Warning + 3 Other intrinseci; Disclosures (fetch/base64/.wasm counts) = dipendenze bundellate, non offuscamento nostro; "scan not available" = gap dello strumento Scorecard in beta, non valutazioni negative. Decisione confermata: **non sventrare le feature** per inseguire uno zero-warning (impossibile per questa categoria) — la Long description del listing già giustifica il modello gated/sicuro. Salvato feedback memory `feedback_dont_gut_features_for_review_score.md`.

### 8. Dependabot
PR [#114](https://github.com/istefox/obsidian-mcp-connector/pull/114) (`vite` 5.4.21→8.0.13 in `packages/test-site`) **chiusa** con commento: `vite` pinnato a `5.4.11` via root `package.json` `overrides` (workspace-wide) → il bump sarebbe inerte + crea mismatch dichiarato/risolto; `test-site` non spedito. Root cause del PR spurio: la `.github/dependabot.yml` v4 (policy: ignore TUTTI i major + total-ignore arktype/svelte/obsidian) era stata **persa nel cleanup de-fork** (referenziava il rimosso `/packages/mcp-server`) → Dependabot a default. **Ripristinata v5** (PR [#117](https://github.com/istefox/obsidian-mcp-connector/pull/117)): policy identica, meno il blocco `mcp-server`, commenti de-forkati, + total-ignore `vite`, `/packages/test-site` deliberatamente non gestito.

### 9. OUTCOME — LISTED
2026-05-16T15:35Z: il plugin è in `obsidianmd/obsidian-releases/community-plugins.json` (id `mcp-tools-istefox`, repo `istefox/obsidian-mcp-connector`) → trovabile e installabile dal browser community **in-app** di Obsidian (breve propagazione CDN). La `description` nel registro è prefissata da Obsidian con "This plugin has not been manually reviewed by Obsidian staff" — standard del path automatico per plugin high-risk (`child_process`/`fs`); è nel file generato da Obsidian (non nel nostro manifest, pulito); rimosso solo da un eventuale audit umano Obsidian (fuori dal nostro controllo, non un difetto). Monitor cron del registro legacy creato e poi eliminato (scopo assolto).

### State change
- `main` HEAD: `a49a3a0` → `f518ef3` (tag `0.4.7`) → `ed19351c` (PR #116, tag `0.4.8`) → `a9204cc9` (PR #117 dependabot restore) → questo handoff (PR docs).
- Release pubblicate: `0.4.7`, `0.4.8` (non-draft, BRAT + portale). `0.4.6` intatto.
- Branch su origin: `main`, `archive/main-0.3.12` (KEEP — rollback 0.3.x). `feat/http-embedded` ritirato.
- PR sessione: #116 (build-repro fix, merged), #117 (dependabot restore, merged), #114 (Dependabot vite — CLOSED); commento su #104.
- Store: portale live + **registro in-app listato** (con disclaimer auto-review). Saga store: **stato terminale**.
- Ruleset `General`: ora targetta solo `~DEFAULT_BRANCH` (main protetto; `feat/http-embedded` rimosso).
- Memoria auto aggiornata in continuo; nuovo `feedback_dont_gut_features_for_review_score.md`. Owner-only residui: nessuno bloccante (fork-network won't-do; smoke Windows #100 non bloccante/field-corroborated).
