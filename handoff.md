...
> **Aggiornato 2026-05-09 mattina (sessione "risolviamo le pending una a una": folotp #54 cross-tracker ack — ruleset General narrow `feat/**` → `feat/http-embedded` esatto + cleanup orphan branch `feat/0.4-migration-ux-cleanup` — FR #88 implementation SHIPPED (PR #92 squash commit `e01617f`, Option A errno-keyed, mock realism update + 2 regression-locked test, plugin suite 747→749 +2 net) — issue #88 chiusa manualmente con close-out comment — passive items #4 marcoaperez (day 4/14 silenzio normale, mock-infra ready) + #5 store #11919 (week 3.6/2-8 quiescenza totale 48h, untouched ✓) confermati. HEAD `feat/http-embedded` = `e01617f` (6 commit avanti vs #87 baseline pre-sessione 2026-05-08). Post-handoff cleanup: orphan branch `fix/73-templates-execute-compat-shim` deleted (8 giorni stale, 3 commit unici tutti già shipped via PR #75 squash `f0ffbfb` — verificato diff 181 ins/5075 del era pure metadata frozen, zero lavoro unico). Final remote: solo `main` + `feat/http-embedded`, zero orphan, zero clutter.**
>
